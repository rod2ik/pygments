"""
    ...
    Lexers for Pseudocode language in French and Spanish Variants.

    :copyright: Copyright 2006-2022 by the Pygments team, and Rodrigo Schwencke, see AUTHORS.
...
__all__ = ['PseudocodeFrLexer', 'PseudocodeEsLexer', 'PythonConsoleLexer', 'PythonTracebackLexer',
...
class PseudocodeFrLexer(RegexLexer):
    """
    For French Pseudocode source code (version 0.1).

    .. versionadded:: 0.10

    .. versionchanged:: 0.10
       This is now the default ``PseudocodeFrLexer``.
    """

    name = 'PseudocodeFr'
    url = 'https://eskool.gitlab.io/mkhack3rs/pseudocode/'
    aliases = ['pseudocodefr', 'pseudofr', 'pseudo', 'algorithme', 'algofr']
    filenames = [
        '*.pseudocodefr',
        '*.pseudofr',
        '*.algofr',
    ...
    mimetypes = ['text/x-pseudocodefr', 'application/x-algobox']
    flags = re.IGNORECASE
    ...
    tokens = {
        'root': [
            ...
            (r'(fonction)((?:\s|\\\s)+)', bygroups(Keyword, Text), 'funcname'),
            (r'(classe)((?:\s|\\\s)+)', bygroups(Keyword, Text), 'classname'),
            (r'(depuis)((?:\s|\\\s)+)', bygroups(Keyword.Namespace, Text),
             'fromimport'),
            (r'(importe)((?:\s|\\\s)+)', bygroups(Keyword.Namespace, Text),
             'import'),
            include('expr'),
        ],
        'expr': [
            ...
            (r'!=|<>|==|<<|>>|:=|[-~+/*%=<>&^|.]', Operator),
            (r'[]{}:(),;[]', Punctuation),
            (r'(dans|est|et|ou|non)\b', Operator.Word),
        ...
        'expr-keywords': [
            ...
            (words((
                'async pour', 'attends', 'sinon', 'pour', 'si', 'anonyme',
                'produit', 'produit depuis'), suffix=r'\b'),
                # 'async for', 'await', 'else', 'for', 'if', 'lambda',
                # 'yield', 'yield from'), suffix=r'\b'),
             Keyword),
            (words(('Vrai', 'VRAI', 'Faux', 'FAUX', 'Vide', 'VIDE'), suffix=r'\b'), Keyword.Constant),
            # (words(('True', 'False', 'None'), suffix=r'\b'), Keyword.Constant),
        ],
        'keywords': [
            (words((
                'supposer', 'async', 'attends', 'arrête', 'continue', 'efface', 'sinonsi', 'sinon si',
                'sinon', 'excepté', 'finalement', 'pour', 'globale', 'si', 'anonyme',
                'passe', 'lève', 'lever', 'nonlocale', 'renvoie', 'renvoyer', 'essaie', 'tente', 'tantque', 'tant que',
                'produit', 'produire',
                'produit depuis', 'produire depuis', 'comme', 'avec'), suffix=r'\b'),
...
Python3Lexer = PseudocodeFrLexer
...
class Python2Lexer(RegexLexer):
    ...
    .. versionchanged:: 2.5
       This class has been renamed from ``PseudocodeFrLexer``.  ``PseudocodeFrLexer`` now
    ...
    aliases = ['python2', 'py2']
    filenames = []  # now taken over by PseudocodeFrLexer (3.x)
...
class PythonConsoleLexer(Lexer):
    ...
    def get_tokens_unprocessed(self, text):
        if self.python3:
            pylexer = PseudocodeFrLexer(**self.options)
...
class PythonTracebackLexer(RegexLexer):
    ...
    tokens = {
        ...
        'intb': [
            ...
            (r'^(    )(.+)(\n)',
             bygroups(Text, using(PseudocodeFrLexer), Text), 'markers'),
            (r'^([ \t]*)(\.\.\.)(\n)',
...
class NumPyLexer(PseudocodeFrLexer):
    """
    ...
    def get_tokens_unprocessed(self, text):
        for index, token, value in \
                PseudocodeFrLexer.get_tokens_unprocessed(self, text):
